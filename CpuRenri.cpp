...
float CpuRenri::convertDigitsToFloat(Digit* digits, int digitCount, int decimalPos) {
  ...
  /*for(int i = 0; i < digitCount; i++){
      if(this->decimalPos-1 < i){
        result = (result * 10) + convertDigit(*(digits+i));
      } else {
        result = result + (static_cast<float>(convertDigit(*(digits+i))) / static_cast<float>(pow(10, numerosDpsVirgula)));
        numerosDpsVirgula++;
      }
    }
    return result;*/
...
void CpuRenri::convertFloatToDigits(Digit* digits, float num) {
    ...
    int decTemp = 0;
  /*while (resultado > 0) {
      this->digit1[this->digit1Count++] = this->convertInt(resultado % 10);
    ...
    while(temp > 0){   // n entendi a logica [inicio]
